module.exports.announcementWriteFunction = (title,description,cataegories,userId,fullName,dateOfPublish) =>
{
    ...
    function updateData(auth) {
      ...
      sheets.spreadsheets.values.update({
        auth: auth,
        spreadsheetId: '1i6Kd38Z-NaO747Wi_DG9bxHH1LCMPzbCiDAIUbBxzYs',
        range: globalRange, //Change Sheet1 if your worksheet's name is something else
        valueInputOption: "USER_ENTERED",
        resource: {
          values: [ [title,description,cataegories,userId,fullName,rowNumber,dateOfPublish]]
        }
      }, (err, response) => {
        if (err) {
